...
**Done:** PRD-1, PRD-2, PRD-3, PRD-4, PRD-5, PRD-6. Suite: **51 tests passing.**
...
## Next
**PRD-7 — Pipeline Run params.** Extend the **`pipeline_run`** doctype JSON at `konsol/pipeline/doctype/pipeline_run/pipeline_run.json` with the run-level parameter fields the orchestrator needs to drive `build_plan`: `fiscal_year`, `fiscal_period`, `scope`, `full_refresh` (Check), `skip_sync` (Check), `pipeline_definition`. These mirror the `params` keys consumed by `plan.build_plan(definition, params)` (see `plan.py`: `skip_sync`, `full_refresh`, `scope`→`select`, `fiscal_year`/`fiscal_period`→`vars`). This is a **doctype JSON PRD — no frappe needed**: write `konsol/tests/test_orchestrator_pipelinerun_doctype.py` (new file) that loads the JSON with `json.load` and asserts each fieldname exists + sanity-checks fieldtypes (`full_refresh`/`skip_sync` should be `Check`; pick sensible types for the rest — `fiscal_year` Int or Data, `fiscal_period` Int or Data, `scope` Data, `pipeline_definition` Data/Select/Link). RED first: assert the new fields are present → fails against current JSON; GREEN: add the `fields` entries. Keep existing fields intact; mind `field_order` if `pipeline_run.json` uses one (PRD-6's `pipeline_step.json` did **not**, but check `pipeline_run.json` separately — it may differ). Do NOT add python/frappe logic. After: regress with `python3 -m pytest konsol/tests/test_orchestrator_*.py -q` (all green), commit with the konsol#57 format, tick PRD-7, update this file.

Tip: read the existing `pipeline_run.json` first (path: `konsol/pipeline/doctype/pipeline_run/pipeline_run.json`) to learn its structure and whether it uses a `field_order` array (if so, append the new fieldnames there too, or the fields won't render). Mirror the surrounding field-dict shape exactly.
