...
**Harness note:** `cd konsol-exec && node --test src/orchestrator/*.test.mjs` runs the pure core suite (13 tests, no npm install needed). Python regress: `python3 -m pytest konsol/tests/test_exec_*.py konsol/tests/test_orchestrator_*.py -q` (now 265 passed / 6 skipped; `test_exec_api.py` exists as of E4). The api client (`startRun`/`getRun`/`retryStep`/`resumeRun`/`cancelRun`/`onRunStep`) is now in place for the machine + components to import.

## Next
**E5 — Exec state machine.** Build the XState machine that drives a full run, importing the pure core (E2/E3) + the api client (E4). This is a **React/machine wiring** PRD → static-assertion pytest (no node --test; the machine pulls XState which isn't host-runnable). Two artifacts:

1. **`konsol-exec/src/machines/runExecMachine.js`** — XState machine, context `{run, error}` (or richer), events `LAUNCH` (invoke `startRun` from `../api` → on done transition to a WATCH/watching state), `REFRESH` and/or realtime → `getRun` then `normalizeRun` (from `../orchestrator/runModel`) to set `context.run`, `RETRY_STEP` (→ `retryStep`), `RESUME_FROM` (→ `resumeRun`), `CANCEL` (→ `cancelRun`), settling to an idle/terminal state when `isTerminal(run.status)` (from `../orchestrator/status`). Keep all data-shaping in the pure core — the machine only orchestrates effects + transitions.
2. **Export it from `konsol-exec/src/machines/index.js`** (check whether that barrel exists; the other machines are `konsolAppMachine.js` / `runDetailMachine.js`).

Test: write `konsol/tests/test_exec_machine.py` — static-assert that `runExecMachine.js` exists, declares the states + the events `LAUNCH`/`REFRESH`/`RETRY_STEP`/`RESUME_FROM`/`CANCEL`, names actors/invokes for `startRun`/`getRun`/`retryStep`/`resumeRun`/`cancelRun`, and imports the pure core (`normalizeRun`, `isTerminal`/`status`) + the api client. RED first (`python3 -m pytest konsol/tests/test_exec_machine.py -q`), implement, GREEN. Regress BOTH suites (`cd konsol-exec && node --test src/orchestrator/*.test.mjs` AND `python3 -m pytest konsol/tests/test_exec_*.py konsol/tests/test_orchestrator_*.py -q`), commit `feat(konsol-exec): E5 exec state machine (konsol#60)`, tick E5 in `PRDS-exec.md`, and repoint this Next section at **E6 — Launch panel** (`konsol-exec/src/components/ExecuteLaunch.jsx`: form fields + `buildRunArgs` + machine `LAUNCH`, static-assert).
