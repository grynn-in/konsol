...
**Harness note:** `cd konsol-exec && node --test src/orchestrator/*.test.mjs` runs the pure core suite (now 13 tests, no npm install needed). Python regress: `python3 -m pytest konsol/tests/test_orchestrator_*.py -q` (254 passed / 5 skipped; no `test_exec_*.py` exist yet — E4+ will add them).

## Next
**E4 — Backend get_run + SPA api client.** This is the first PRD needing a `konsol/tests/test_exec_*.py` static-assert test PLUS a frappe-guarded python test. Two parts:

1. **Backend** — add a whitelisted `get_run(run_name)` to `konsol/orchestrator/api.py` returning `{name, status, steps:[...]}` (the steps child rows with `step_id, step_type, status, started_at, ended_at, rows, output, error`). Guard the python test with `import pytest; frappe = pytest.importorskip("frappe")`.
2. **SPA api client** — in `konsol-exec/src/api.js` add `startRun(definition, params)`, `getRun(name)`, `retryStep(name, stepId)`, `resumeRun(name, stepId)`, `cancelRun(name)` each delegating to `frappeCall("konsol.orchestrator.api.<fn>", …)` (start_run/get_run/retry_step/resume_run/cancel_run), plus `onRunStep(cb)` wrapping `frappe.realtime?.on("orchestrator_step", cb)` (guard when realtime absent).

Test: write `konsol/tests/test_exec_api.py` — a static-assertion pytest that reads `konsol-exec/src/api.js` and asserts each fn exists and references the right backend method string (mirrors `test_orchestrator_spa_js.py` convention); plus a frappe-guarded python test for `get_run`. RED first (`python3 -m pytest konsol/tests/test_exec_*.py -q`), then implement, GREEN. Regress both suites, commit `feat(konsol-exec): E4 backend get_run + SPA api client (konsol#60)`, tick E4 in `PRDS-exec.md`, and repoint this Next section at **E5 — Exec state machine** (`konsol-exec/src/machines/runExecMachine.js`, static-assert states/events/actors + imports of the pure core + api).
